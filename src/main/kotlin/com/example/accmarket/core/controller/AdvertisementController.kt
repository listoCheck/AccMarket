...
class AdvertisementController(
    val advertisementService: AdvertisementService, private val jwtProvider: JwtProvider
) {
    ...
    @Operation(
        ...
        requestBody = SwaggerRequestBody(
            required = true, content = [Content(schema = Schema(implementation = AdvertisementDTO::class))]
        ),
        responses = [ApiResponse(
            ...
            responseCode = "400",
            description = "Объявление содержит запрещенные слова или пользователь/токен не найден",
            content = [Content(schema = Schema(implementation = Response::class))]
        )]
    )
    ...
    @Operation(
        ...
        responses = [ApiResponse(
            ...
            responseCode = "403",
            description = "Нет доступа к редактированию объявления",
            content = [Content(schema = Schema(implementation = Response::class))]
        ), ApiResponse(
            responseCode = "404",
    ...
    @Operation(
        ...
        requestBody = SwaggerRequestBody(
            required = true, content = [Content(schema = Schema(implementation = DeleteAdvertisementDTO::class))]
        ),
        responses = [ApiResponse(
            responseCode = "200",
            ...
            responseCode = "403",
            description = "Нет доступа к удалению объявления",
            content = [Content(schema = Schema(implementation = Response::class))]
        ), ApiResponse(
            responseCode = "404",
            description = "Объявление не найдено",
    ...
    @GetMapping("/cabinet/seller")
    @Operation(
        summary = "Объявления продавца",
        description = "Возвращает все объявления текущего пользователя с любыми статусами",
        responses = [ApiResponse(
            responseCode = "200",
    ...
    fun myCreated(@RequestHeader("Authorization") token: String) =
        advertisementService.getByCreator(jwtProvider.getUserId(token))


    @GetMapping("/cabinet/buyer")
    @Operation(
        summary = "Купленные объявления",
        description = "Возвращает купленные объявления с логином и паролем от аккаунта",
        responses = [ApiResponse(responseCode = "200", description = "Список купленных аккаунтов")]
    )
    fun myBought(@RequestHeader("Authorization") token: String) =
        advertisementService.getBought(jwtProvider.getUserId(token))


    @PostMapping("/buy")
    @Operation(
        summary = "Покупка объявления",
        description = "Покупка объявления с автоматическим списанием баланса и отправкой данных аккаунта",
        requestBody = SwaggerRequestBody(
            required = true, content = [Content(schema = Schema(implementation = BuyAdvertisementDTO::class))]
        ),
        responses = [ApiResponse(
            responseCode = "200",
            description = "Объявление успешно куплено",
            content = [Content(schema = Schema(implementation = Response::class))]
        ), ApiResponse(
            responseCode = "400",
            description = "Недостаточно средств или неверный статус объявления"
        ), ApiResponse(responseCode = "403", description = "Нельзя купить собственное объявление"), ApiResponse(
            responseCode = "404",
            description = "Объявление не найдено"
        )]
    )
    fun buyAdvertisement(
        @RequestHeader("Authorization") token: String, @RequestBody dto: BuyAdvertisementDTO
    ): Response {
        return advertisementService.buy(dto, token)
    }
