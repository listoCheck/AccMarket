...
## Подробные описания прецедентов

### Прецедент: Зарегистрироваться / войти
- **ID:** 1  
- **Краткое описание:** Пользователь заполняет форму регистрации или авторизации и входит в систему.  
- **Предусловия:** Пользователь хочет зарегистрироваться или войти.  
- **Основной поток:**  
  1) Пользователь открывает форму входа/регистрации.  
  2) Вводит логин и пароль.  
  3) Нажимает кнопку «Войти».  
  4) Система проверяет данные и авторизует.  
...
### Прецедент: Арендовать аккаунт
- **ID:** 2  
- **Краткое описание:** Пользователь арендует аккаунт у продавца.  
- **Предусловия:** Пользователь авторизован и имеет достаточный баланс.  
- **Основной поток:**  
  1. Пользователь выбирает объявление.  
  2. Нажимает «Арендовать».  
  3. Система списывает деньги.  
  4. Система выдаёт временный доступ.  
- **Альтернативный поток:**  
  - Недостаточно средств → система сообщает об ошибке.  
- **Постусловия:** Пользователь получает временный доступ.  

---

### Прецедент: Купить аккаунт
- **ID:** 3  
- **Краткое описание:** Пользователь приобретает аккаунт у продавца.  
- **Предусловия:** Пользователь авторизован и имеет достаточный баланс.  
- **Основной поток:**  
  1. Пользователь выбирает объявление.  
  2. Нажимает «Купить».  
  3. Система списывает деньги.  
  4. Система передаёт данные аккаунта.  
- **Альтернативный поток:**  
  - Недостаточно средств → ошибка.  
- **Постусловия:** Аккаунт передан пользователю, деньги продавцу.  

---

### Прецедент: Выставить аккаунт на продажу
- **ID:** 4  
- **Краткое описание:** Продавец размещает объявление о продаже аккаунта.  
- **Предусловия:** Продавец авторизован.  
- **Основной поток:**  
  1. Продавец создаёт новое объявление.  
  2. Заполняет данные об аккаунте.  
  3. Отправляет объявление.  
- **Альтернативный поток:**  
  - Некорректные данные → объявление не создаётся.  
- **Постусловия:** Создано активное объявление.  

---

### Прецедент: Сдать аккаунт в аренду
- **ID:** 5  
- **Краткое описание:** Продавец сдаёт аккаунт во временное пользование.  
- **Предусловия:** Продавец авторизован.  
- **Основной поток:**  
  1. Продавец создаёт объявление «Аренда».  
  2. Заполняет условия.  
  3. Отправляет на модерацию.  
- **Альтернативный поток:**  
  - Ошибки в данных → объявление не создаётся.  
- **Постусловия:** Аккаунт доступен для аренды.  

---

### Прецедент: Предоставить семейный доступ
- **ID:** 6  
- **Краткое описание:** Продавец делится своей библиотекой игр.  
- **Предусловия:** Продавец авторизован.  
- **Основной поток:**  
  1. Продавец выбирает «Семейный доступ».  
  2. Указывает пользователя.  
  3. Система открывает доступ.  
- **Альтернативный поток:**  
  - Ошибка предоставления доступа.  
- **Постусловия:** Пользователь получает доступ к библиотеке.  

---

### Прецедент: Пополнить баланс
- **ID:** 7  
- **Краткое описание:** Пользователь увеличивает баланс кошелька.  
- **Предусловия:** Пользователь авторизован.  
- **Основной поток:**  
  1. Пользователь выбирает метод пополнения.  
  2. Вводит сумму.  
  3. Производит оплату.  
- **Альтернативный поток:**  
  - Ошибка платежа.  
- **Постусловия:** Баланс пополнен.  

---

### Прецедент: Проверить объявление
- **ID:** 8  
- **Краткое описание:** Модератор проверяет валидность объявления.  
- **Предусловия:** Объявление создано.  
- **Основной поток:**  
  1. Модератор открывает новое объявление.  
  2. Проверяет корректность данных.  
  3. Одобряет или отклоняет.  
- **Альтернативный поток:**  
  - Объявление некорректное - отклонено.  
- **Постусловия:** Объявление имеет статус «одобрено» или «отклонено».  

---

### Прецедент: Просмотреть историю операций
- **ID:** 9  
- **Краткое описание:** Пользователь просматривает историю действий.  
- **Предусловия:** Пользователь авторизован.  
- **Основной поток:**  
  1. Пользователь открывает «Историю».  
  2. Система отображает список операций.  
- **Альтернативный поток:**  
  - Нет операций - отображается пустой список.  
- **Постусловия:** Пользователь получил историю операций.  

---

### Прецедент: Получить уведомление о сделке
- **ID:** 10  
- **Краткое описание:** Пользователь получает уведомление об изменении статуса сделки.  
- **Предусловия:** Пользователь участвует в сделке.  
- **Основной поток:**  
  1. Система фиксирует событие.  
  2. Отправляет уведомление пользователю.  
- **Альтернативный поток:**  
  - Сбой отправки - уведомление не доставлено.  
- **Постусловия:** Пользователь проинформирован.  

---

### Прецедент: Блокировка пользователя
- **ID:** 11  
- **Краткое описание:** Модератор блокирует мошенника.  
- **Предусловия:** Пользователь нарушил правила.  
- **Основной поток:**  
  1. Модератор выбирает пользователя.  
  2. Выбирает «Заблокировать».  
  3. Система блокирует доступ.  
- **Альтернативный поток:**  
  - Ошибка блокировки.  
- **Постусловия:** Пользователь лишён доступа.  

---

### Прецедент: Апелляция блокировки
- **ID:** 12  
- **Краткое описание:** Пользователь подаёт жалобу на блокировку.  
- **Предусловия:** Пользователь заблокирован.  
- **Основной поток:**  
  1. Пользователь открывает форму апелляции.  
  2. Заполняет обращение.  
  3. Модератор рассматривает и принимает решение.  
- **Альтернативный поток:**  
  - Апелляция отклонена.  
- **Постусловия:** Пользователь либо восстановлен, либо остаётся заблокированным.  
